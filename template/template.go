...
func Merge(obj map[string]interface{}, format StringFormat, skipArray bool, separator string, prefix string, suffix string) TStatement {
	...
	if len(separator) > 0 && len(parameters) == 1 {
		...
		if vo.Kind() == reflect.Slice {
			...
			if l > 0 {
				...
				return TStatement{Query: prefix + strings.Join(results, "") + suffix, Params: params}
			}
		}
	}
	texts := format.Texts
...
func Build(obj map[string]interface{}, template Template) (string, []interface{}) {
	results := make([]string, 0)
	params := make([]interface{}, 0)
...
func UseQuery(isTemplate bool, query func(interface{}) (string, []interface{}), id string, m map[string]*Template, modelType *reflect.Type, mp func(interface{}, *reflect.Type, ...func(string, reflect.Type) string) map[string]interface{}, buildSort func(string, reflect.Type) string, opts ...func(string) string) (func(interface{}) (string, []interface{}), error) {
	if !isTemplate {
		return query, nil
	}
