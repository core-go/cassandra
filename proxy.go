...
func (c *ProxyClient) Exec(ctx context.Context, query string, values ...interface{}) (int64, error) {
	stm := BuildStatement(query, values...)
	var r int64
	err := PostWithClientAndDecode(ctx, c.Client, c.Url+"/exec", stm, &r, c.Log)
	return r, err
}
func (c *ProxyClient) ExecBatch(ctx context.Context, master bool, stm ...Statement) (int64, error) {
	stmts := BuildJStatements(stm...)
	if len(stmts) == 0 {
		return 0, nil
	}
	sm := ""
...
func (c *ProxyClient) ExecBatchWithTx(ctx context.Context, tx string, commit bool, master bool, stm ...Statement) (int64, error) {
	stmts := BuildJStatements(stm...)
	if len(stmts) == 0 {
		return 0, nil
	}
